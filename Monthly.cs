...
namespace budgetCalculator
{
    public partial class Monthly : Form
    {
        ...
        private void GenerateSchedule()
        {
            ...
            // Call DisplaySchedule to update the DataGridView with the latest schedule
        ...
        private void DisplaySchedule(double totalEnergy, double totalCost)
        {
            ...
            // Group the schedule items by the day of the week
            var groupedByDay = schedule.GroupBy(item => item.DayOfWeek).OrderBy(group => group.Key);

            // Get today's date and start from the next day
            DateTime currentDate = DateTime.Today.AddDays(1); // Start from tomorrow

            // Iterate over each day group
            foreach (var dayGroup in groupedByDay)
            {
                // Add a row with the current date
                dgvSchedule.Rows.Add(currentDate.ToString("dddd, MMM dd yyyy"));

                // Add each appliance's schedule for that day
                foreach (var item in dayGroup)
                {
                    dgvSchedule.Rows.Add(
                        "", // Empty cell under "Date" for appliance rows
                ...
                // Add a blank row after each day's schedule for visual separation
                dgvSchedule.Rows.Add();

                // Increment the current date for the next day's schedule
                currentDate = currentDate.AddDays(1);
            }

            // Update the totals displayed on the form
